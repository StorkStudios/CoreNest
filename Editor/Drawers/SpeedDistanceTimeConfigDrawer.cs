    ...
    public class SpeedDistanceTimeConfigDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (property.isExpanded)
            {
                ...
                using (new EditorGUI.DisabledGroupScope(modeIndex == 0))
                {
                    EditorGUI.PropertyField(fieldRect, speed);
                    fieldRect.y += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;
                }
                using (new EditorGUI.DisabledGroupScope(modeIndex == 1))
                {
                    EditorGUI.PropertyField(fieldRect, distance);
                    fieldRect.y += EditorGUIUtility.singleLineHeight + EditorGUIUtility.standardVerticalSpacing;
                }
                using (new EditorGUI.DisabledGroupScope(modeIndex == 2))
                {
